...
def sequential_batch_sampler(
    ...
    num_batches = num_tokens // total_batch_size
    
    for i in range(num_batches):
        start_idx = i * total_batch_size
        end_idx = start_idx + total_batch_size
        
        # Reshape the flat sequence into (batch_size, seq_len)
...
def compute_language_modeling_loss(
    ...
    # So labels are input_ids shifted by 1 position
    labels = input_ids[:, 1:].contiguous()  # Remove first token (we don't predict before it)
    logits = logits[:, :-1, :].contiguous()  # Remove last prediction (no label for it)
